...
let pkg_config pkgs args =
  let pkg_config_fallback = try ":" ^ (Sys.getenv "PKG_CONFIG_PATH") with Not_found -> "" in
  ...
  and value = Fmt.strf "%s/share/pkgconfig:%s/lib/pkgconfig%s" prefix prefix pkg_config_fallback
  in
